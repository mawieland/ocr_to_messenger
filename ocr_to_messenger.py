...
while test_run:
    ...
    test_ocr_string = pytesseract.image_to_string(test_hsv_image)
    print(test_ocr_string)
...
def spam_user_if_no_queue(bot, job):
...
def start_alarm(bot, update, job_queue, chat_data):
    ...
    job = job_queue.run_repeating(spam_user_if_no_queue, ALERT_SPAM_INTERVAL, context=chat_id)
    chat_data['job'] = job
...
start_alarm_handler = CommandHandler('startalarm',
                                     ...
                                     pass_job_queue=True,
                                     pass_chat_data=True)
stop_alarm_handler = CommandHandler("stopalarm", stop_alarm, pass_chat_data=True)
